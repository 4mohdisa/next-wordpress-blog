...
export function PostCard({ post, options = {} }: PostCardProps) {
  ...
  return (
    <Card
      className={`relative transition-all bg-white dark:bg-black hover:border-link-default dark:hover:border-link-hover ${isSticky ? 'border-link-default dark:border-link-hover rounded-xl' : 'border-border-default'}`}
    >
      <CardHeader className="relative p-4 sm:p-6">
        {isSticky && (
          <MapPin
            className="absolute right-2 top-2 h-4 w-4 text-black dark:text-white opacity-60"
